...
namespace Core.Parallax
{
    public class InfiniteParallaxLayer 
    {
        ...
        public InfiniteParallaxLayer(SpriteRenderer initialPart, float speed, Transform parentTransform, Transform initialPartTransform)
        {
            _speed = speed;
            Sprite sprite = initialPart.sprite;
            _layerHorizontalSize = sprite.texture.width / sprite.pixelsPerUnit * initialPartTransform.transform.localScale.x;
        ...
        public void UpdateLayer(float targetPosition, float previousPosition)
        {
            MovePart(previousPosition - targetPosition);
            FixLayerPosition(targetPosition);
        }

        private void MovePart(float deltaMovement)
        {
        ...
        private void FixLayerPosition(float targetPosition)
        {
